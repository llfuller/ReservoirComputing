...
def build_and_train_and_predict(Group_obj, perform_grid_search, start_time,train_start_timestep,train_end_timestep,mse_array,
                                ...
                                setup_number, random_initial_condition_list, pnz, res_states_list, param_array):
    ...
    for IC_index, initial_condition in enumerate(random_initial_condition_list):
        for ESN_obj in Group_obj.list_of_ESN_objs:
            ESN_obj.x = np.zeros(np.shape(ESN_obj.x))
            ESN_obj.x[0] = initial_condition
        ...
        for l, beta in enumerate(list_of_beta_to_test):
            ...
            # Don't do this because we want to see if prediction state has generally synchronized
            ...
            # When checking gen sync, no prediction needed
            ...
            # When checking gen sync, no prediction needed
            # Plotting.plot_1D_quick(Group_obj.list_of_ESN_objs[0].x[0:train_end_timestep,0:10])
            np.savetxt("states/" + system_name + "/" +
                   "prediction_gen_synch/index_"+str(IC_index+1)+"/"+dimension_directory+
                   "_orbit_params_(" +
                   ...
                   Group_obj.list_of_ESN_objs[0].x[0:train_end_timestep,:], fmt='%.3e')
            res_states_list[IC_index] = Group_obj.list_of_ESN_objs[0].x[0:train_end_timestep,:]