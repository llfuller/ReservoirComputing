...
class ESN_GPU: # TODO: Speed this up! Parallelize or use sparse?
    # connection adjacency matrix W (nonlocal):
    # W is a cupy array
    W = sp.zeros((1,1))
    # reservoir nodes' activations:
    ...
    def calculate_W_out(self, Y_target, N_x, beta, train_start_timestep, train_end_timestep):
        # see Lukosevicius Practical ESN eqtn 11
        # Using ridge regression
        N_u = sp.array(sp.shape(Y_target)[0])
        X = sp.array(sp.vstack((sp.ones((1,train_end_timestep-train_start_timestep)),
                       Y_target[:,train_start_timestep:train_end_timestep],
                       self.x[train_start_timestep:train_end_timestep].transpose())))
        # Ridge Regression
        W_out = cp.array(sp.matmul(sp.array(Y_target[:, train_start_timestep+1:train_end_timestep+1]),
                          sp.matmul(X.transpose(),
                                    sp.linalg.inv(sp.matmul(X,X.transpose()) + beta*sp.identity(1+N_x+N_u)))))
        # plt.figure()
        # plt.plot(X)
    ...
    def output_Y(self, u, n):
        ...
        one_by_one = cp.array([1])
        # concatinated_matrix has dimension (231,)
        # Eqtn 4
        concatinated_matrix = cp.hstack((cp.hstack((one_by_one,
                                                    cp.array(u))),
                                         cp.array(self.x[n])))
        output_Y_return = cp.asnumpy(cp.matmul(self.W_out, concatinated_matrix))
        endoutput = time.time()
        # print("Output_Y runs in "+str((time.time() - start_output)/1000.0) +" seconds")
        return output_Y_return


class ESN_GPU_2: # TODO: Speed this up! Parallelize or use sparse?
    # connection adjacency matrix W (nonlocal):
    # W is a cupy array
    W = sp.zeros((1,1))
    # reservoir nodes' activations:
    ...
    def calculate_W_out(self, Y_target, N_x, beta, train_start_timestep, train_end_timestep):
        ...
        X = cp.array(sp.vstack((sp.ones((1,train_end_timestep-train_start_timestep)),
                       ...
                       self.x[train_start_timestep:train_end_timestep].transpose())))
        # Ridge Regression
        print("1+N_x+N_u is "+str(1+N_x+N_u))
