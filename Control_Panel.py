...
timesteps_for_prediction = 10000
save_or_display = "save" #save 3d plots of orbits after prediction or display them
...
def build_and_train_and_predict(start_time,train_start_timestep,train_end_timestep,mse_array,beta,
                N_u,N_y,N_x,x_initial,Y_target, scaling_W_fb, timesteps_for_prediction, scaling_W_in, system_name):
    ...
    ESN_1.calculate_W_out(Y_target, N_x, beta, train_start_timestep,
                          train_end_timestep)
    print_timing(print_timings_boolean, start_time, "after_W_out_train_time")

    # Predict Y at each next timestep, keeping training progress (This isn't training.
    # This is prediction):
    Y[:, 0:train_end_timestep+1] = Y_target[:, 0:train_end_timestep+1]
    for n in range(train_end_timestep, train_end_timestep + timesteps_for_prediction-1):
        Y[:, n+1] = ESN_1.output_Y(Y[:, n], n)
        ESN_1.update_reservoir(Y[:, n], n, Y[:,n+1])
    ...
    # Plotting.plot_activations(Y, ESN_1.x)
    Plotting.plot_3D_orbits(Y, Y_target, train_end_timestep, system_name,
...
extra_W_in_scale_factor_grid = np.float32(range(1,10))/5.0 # input scalings grid
scaling_W_grid = np.float32(range(1,5))/5.0 # direct multiplier for spectral radius grid after normalization occurs
alpha_grid = np.float32(range(1,6))/5.0 # uniform leaking rate grid
# Secondary parameters to grid search
beta_grid = np.logspace(0, 6, 6)
...
if system_name is "L96":
    Lorenz96.run_L96(t_final = 1000.0,
                     dt = 0.001)
    N_u, N_y = 36, 36
    Y_target = (np.loadtxt('L96_States.txt')).transpose()
    num_timesteps_data = np.shape(Y_target)[1]
...
Y_target = np.divide(Y_target,np.max(np.abs(Y_target)))
...
if perform_grid_search:
        ...
        for j, scaling_W in enumerate(scaling_W_grid):
            for k, scaling_alpha in enumerate(alpha_grid):
                # for l, beta in enumerate(beta_grid):
                    ...
                    build_and_train_and_predict(start_time, train_start_timestep, train_end_timestep,
                                                mse_array, beta, N_u, N_y, N_x, x_initial, Y_target,
    ...
    build_and_train_and_predict(start_time, train_start_timestep, train_end_timestep, mse_array, beta,
                                N_u, N_y, N_x, x_initial, Y_target, scaling_W_fb,
                                timesteps_for_prediction, scaling_W_in, system_name)
