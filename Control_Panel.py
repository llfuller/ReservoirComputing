...
data_storage_number_list = np.arange(0,20,1)
MSE_from_trial_list = 1000*np.ones(len(data_storage_number_list))
for data_storage_number in data_storage_number_list:
    # for system_name in ["L96"]:
    #     for setup_number in [1]:
    #         for dims in [5,6,8,9,36]:
    # print("Running for "+str(dims)+" dims")
    np.random.seed(data_storage_number)
    #=======================================================================================================================
    # Run Parameters
    #=======================================================================================================================
    ...
    timesteps_for_prediction = 30000 # if this is too big, then calculations take too long. Medium range: MSE is meaningless. Short range: good measure
    ...
    for var_index in range(dims): #new addition to code
        state_target[var_index] = np.divide(state_target[var_index],np.max(np.abs(state_target[var_index]))) # Actual normalized input to reservoir.
            ...
            for k, scaling_alpha in enumerate(list_of_scaling_alpha):
                for m, scaling_W_fb in enumerate(list_of_scaling_W_fb):
                    ...
                    ESN_Process.build_and_train_and_predict(Group_1, perform_grid_search,
                                                            ...
                                                            setup_number, extra_stuff_list, data_storage_number,
                                                            param_array=[i,j,k,m])
    MSE_from_trial_list[data_storage_number] = 1.0/timesteps_for_prediction*np.mean(np.square(np.abs(np.subtract(state[:,train_end_timestep:train_end_timestep + timesteps_for_prediction],state_target[:,train_end_timestep:train_end_timestep + timesteps_for_prediction]))))
    plt.figure()
    plt.plot(state_target[0,train_end_timestep:train_end_timestep + timesteps_for_prediction].transpose(), color = 'b')
    plt.plot(state[0,train_end_timestep:train_end_timestep + timesteps_for_prediction].transpose(), color = 'orange', linestyle = ':')
    plt.show()
...
np.savetxt("MSE_from_trials.txt",MSE_from_trial_list)